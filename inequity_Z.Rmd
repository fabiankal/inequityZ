...
data_N_ggplot2$country <- countrycode(data_N_ggplot2$cntry, "iso2c", "country.name")
#dataset %>% group_by(cntry) %>% tally() %>% summarise (ntot=sum(n)) #all in the sample
#dataset %>% filter(pdwrk == 1) %>% group_by(cntry) %>% tally() %>% summarise (ntot=sum(n)) #only those working
...
#calculate the average fairness ratings by income decile and country
...
# option: export table including the number of observations by country (see appendix)
# data_plot_long_N1<-data_long %>% 
#  group_by(cntry,condition) %>% tally()
# write.csv(data_plot_long_N1, file.path(projDir,"output_Z/gross_fairn_cntryP90P10_dec_cases_alt.csv"), na = "NA")
...
data_plot_long_mean1$country <- countrycode(data_plot_long_mean1$cntry, "iso2c", "country.name")

data_plot_long_mean1$cntryP90P10_n2<-as.factor(data_plot_long_mean1$cntryP90P10_n2)
levels(data_plot_long_mean1$cntryP90P10_n2)<-c("cntryP90P10_n2#1","cntryP90P10_n2#2","cntryP90P10_n2#3","cntryP90P10_n2#4","cntryP90P10_n2#5","cntryP90P10_n2#6")

data_plot_long_mean1$rank<-as.factor(data_plot_long_mean1$rank)
levels(data_plot_long_mean1$rank)<-c("rank#1","rank#2","rank#3","rank#4","rank#5")

hospital_names <- c(
                    `rank#1` = "lower level of\nbottom incomes",
                    `rank#2` = "",
                    `rank#3` = "",
                    `rank#4` = "",
                    `rank#5` = "higher level of\nbottom incomes",
                    
                    `cntryP90P10_n2#1` = "1 - low inequality",
                    `cntryP90P10_n2#2` = "2",
                    `cntryP90P10_n2#3` = "3",
                    `cntryP90P10_n2#4` = "4",
                    `cntryP90P10_n2#5` = "5",
                    `cntryP90P10_n2#6` = "6 - high inequality"
                    )
...
p<-ggplot(data=data_plot_long_mean1, aes(x=grspnum_ft_i_decile,y=mean_measurement,color=condition)) +
  ...
  geom_text(aes(label=country),stat = 'identity',x=5.5,y=3.5,size=3,inherit.aes = FALSE,check_overlap = TRUE) +
  geom_line(aes(group=condition, linetype=condition)) +
  geom_ribbon(aes(ymin=ymin,ymax=ymax,fill=condition),alpha=0.3, colour = NA) +
  facet_grid(cntryP90P10_n2~rank, labeller = as_labeller(hospital_names))+
  ...
  scale_x_continuous(breaks=1:10,
                     sec.axis = sec_axis(~ . , name = expression("PPP-adjusted level of bottom incomes within the inequality group" %->% "")
                                         , breaks = NULL, labels = NULL)) + 
  scale_y_continuous(limits=c(-4.1,4),breaks=c(-2,0,2),
                     sec.axis = sec_axis(~ . , name = expression("P90/P10 income inequality group " %->% "")
                                         , breaks = NULL, labels = NULL)) +
...
# export the graph using different formats
ggsave(file.path(projDir,"output_Z/figure_1.png"),g, units="in", width=9, height=9, type="cairo-png", dpi=320)
ggsave(file.path(projDir,"output_Z/figure_1.tiff"),g, units="in", width=9, height=9, dpi=320)
ggsave(file.path(projDir,"output_Z/figure_1.pdf"),g, units="in", width=9, height=9)
ggsave(file.path(projDir,"output_Z/figure_1.svg"),g, units="in", width=9, height=9)
```
...
The full replication code to rebuild the visualization is available [here](https://github.com/fabiankal/inequityZ).
